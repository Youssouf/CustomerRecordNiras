 ...
 public  class AccountHelper
    {
        ...
        private static List<SelectListItem> _dayOfBirth = new List<SelectListItem>();
        private static List<SelectListItem> _monthOfBirth = new List<SelectListItem>();
        public static List<SelectListItem> _gender = new List<SelectListItem>();
        private static List<SelectListItem> _companyName = new List<SelectListItem>();
        private static List<SelectListItem> _departmentName = new List<SelectListItem>();
        ...
        public static List<SelectListItem> DayOfBirth
        {
            get
            {
                if (_dayOfBirth.Count==0)
                {
                    _dayOfBirth.Add(new SelectListItem { Text = "Day", Value = "0" });
                }
                else
                {
                    for (int i = 0; i < 32; i++)
                    {
                        _dayOfBirth.Add(new SelectListItem { Text = i.ToString(), Value = i.ToString() });
                    }
                }
                return _dayOfBirth;
            }
        }
        public static List<SelectListItem> MonthOfBirth
        {
            get
            {
                if (_monthOfBirth.Count == 0)
                {
                    _monthOfBirth.Add(new SelectListItem { Text = "Year", Value = "0" });
                }
                else
                {
                    for ( int year = DateTime.Now.Year ; year > year-130; year--)
                    {
                        _monthOfBirth.Add(new SelectListItem { Text = Convert.ToString(year), Value = Convert.ToString(year) });
                    }
                }
